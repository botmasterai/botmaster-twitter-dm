...
class TwitterDMBot extends BaseBot {
  ...
  __sendMessage(rawMessage, send_options, quick_reply = false) {
    ...
    if (quick_reply) {
      return new Promise((resolve, reject) => {
        this.twit.post('direct_messages/events/new', rawMessage, (err, data) => {
          if (err) {
            reject(err);
          }
          resolve(data);
  ...
  __createStandardBodyResponseComponents(sentOutgoingMessage, sentRawMessage, rawBody) {
    // if: to capture normal text based response from twitter
    // else: to capture the response of quick reply buttons
